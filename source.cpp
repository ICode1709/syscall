...
PLARGE_INTEGER BaseFormatTimeOut(OUT PLARGE_INTEGER Timeout, IN ULONG dwMilliseconds)
{
...
VOID NTAPI RtlInitUnicodeString(IN PUNICODE_STRING DestinationString, IN PCWSTR SourceString)
{
...
VOID NTAPI RtlInitAnsiString(IN PANSI_STRING DestinationString, IN PCSTR SourceString)
{
	SIZE_T Size;
...
PVOID GetModuleBase(PPEB Peb, PCWSTR ModuleName)
{
	...
	if (Peb)
	{
		if (ModuleName)
		{
			for (PLIST_ENTRY ListEntry = Peb->Ldr->InInitializationOrderModuleList.Flink; ListEntry != &Peb->Ldr->InInitializationOrderModuleList; ListEntry = ListEntry->Flink)
			{
				...
				if (wcscmp(LdrEntry->BaseDllName.Buffer, ModuleName) == 0)
				{
...
PVOID GetProcedureAddress(PVOID ImageBase, PCSTR ImportName)
{
...
PVOID GetFunctionAddress(PPEB Peb, PCWSTR ModuleName, PCSTR FunctionName)
{
...
#define THREAD_CREATE_FLAGS_INITIAL_THREAD 0x00000080
	...
	NTSTATUS CreateThread(OUT PHANDLE ThreadHandle, IN HANDLE ProcessHandle, IN ULONG DesiredAccess, IN PVOID StartAddress, IN PVOID Parameter,
	...
	PVOID VirtualAllocEx(IN HANDLE ProcessHandle, IN PVOID BaseAddress, IN ULONG_PTR RegionSize, IN ULONG AllocationType, IN ULONG Protect)
	{
	...
	PVOID VirtualAlloc(IN PVOID BaseAddress, IN ULONG_PTR RegionSize, IN ULONG AllocationType, IN ULONG Protect)
	{
		return VirtualAllocEx(GetCurrentProcess(), BaseAddress, RegionSize, AllocationType, Protect);
	}
	BOOLEAN VirtualProtectEx(HANDLE ProcessHandle, PVOID BaseAddress, ULONG_PTR RegionSize, ULONG NewProtect, PULONG OldProtect)
	{
		return NtProtectVirtualMemory(ProcessHandle, &BaseAddress, &RegionSize, NewProtect, OldProtect) == STATUS_SUCCESS;
	}
	BOOLEAN VirtualProtect(PVOID BaseAddress, ULONG_PTR RegionSize, ULONG NewProtect, PULONG OldProtect)
	{
		return VirtualProtectEx(GetCurrentProcess(), BaseAddress, RegionSize, NewProtect, OldProtect);
	}

	VOID Sleep(ULONG dwMilliseconds)
	{
		LARGE_INTEGER Time;
		NtDelayExecution(FALSE, BaseFormatTimeOut(&Time, dwMilliseconds));
	}
	NTSTATUS NTAPI SetPrivilege(IN ULONG Privilege, IN ULONG Attributes)
	{
	...
	NTSTATUS CreateFile(PHANDLE FileHandle, PCWSTR filename, ULONG DesiredAccess, ULONG FileAttributes, ULONG ShareAccess, ULONG CreateDisposition, ULONG CreateOptions, PVOID EaBuffer, ULONG EaLength)
	{
	...
	NTSTATUS WriteFile(HANDLE FileHandle, PVOID Buffer, ULONG_PTR Offset, ULONG_PTR Length, PULONG_PTR pWritted)
	{
	...
	NTSTATUS DeleteFileEx(IN HANDLE FileHandle)
	{
	...
	NTSTATUS DeleteFile(IN PCWSTR FilePath, IN BOOLEAN Close = TRUE)
	{
		/*
	...
	NTSTATUS MoveFileEx(IN HANDLE FileHandle, IN PCWSTR NewFileName, IN BOOLEAN ReplaceIfExists)
	{
	...
	NTSTATUS MoveFile(IN PCWSTR ExistingFileName, IN PCWSTR NewFileName, IN BOOLEAN ReplaceIfExists)
	{
		HANDLE FileHandle = NULL;
	...
	NTSTATUS SetFileAttributes(IN HANDLE FileHandle, IN ULONG FileAttributes)
	{
	...
	NTSTATUS OpenProcess(OUT PHANDLE ProcessHandle, IN ULONG DesiredAccess, IN BOOLEAN InheritHandle, IN HANDLE UniqueProcessId)
	{
		OBJECT_ATTRIBUTES ObjectAttributes;
	...
	NTSTATUS OpenThread(OUT PHANDLE ThreadHandle, IN ULONG DesiredAccess, IN BOOLEAN InheritHandle, IN HANDLE UniqueThreadId)
	{
		OBJECT_ATTRIBUTES ObjectAttributes;
		InitializeObjectAttributes(&ObjectAttributes, NULL, DesiredAccess ? OBJ_INHERIT : NULL, NULL, NULL);
	...
	NTSTATUS DeviceIoControl(IN HANDLE Device, IN ULONG IoControlCode, IN OPTIONAL PVOID InBuffer, IN ULONG InBufferSize, OUT OPTIONAL PVOID OutBuffer, IN ULONG OutBufferSize, OUT OPTIONAL PULONG BytesReturned, IN OUT OPTIONAL LPOVERLAPPED Overlapped)
	{
	...
	NTSTATUS CreateFileFromMemory(IN PCWSTR FilePath, IN PVOID Buffer, IN ULONG_PTR Length, IN ULONG FileAttributes)
	{
		HANDLE FileHandle;
	...
	NTSTATUS OpenKey(OUT PHANDLE KeyHandle, HANDLE RootKeyHandle, IN ACCESS_MASK DesiredAccess, IN PCWSTR Key, IN ULONG OpenOptions)
	{
		OBJECT_ATTRIBUTES ObjectAttributes;
	...
	NTSTATUS CreateKey(OUT PHANDLE KeyHandle, IN HANDLE RootKeyHandle, IN ULONG Desired, IN PCWSTR SubKey, IN PCWSTR Class, IN ULONG Options, OUT OPTIONAL PULONG Disposition)
	{
		UNICODE_STRING UnicodeObjectName;
	...
	NTSTATUS SetValueKey(IN HANDLE KeyHandle, IN PCWSTR ValueName, IN OPTIONAL ULONG TitleIndex, IN ULONG Type, IN OPTIONAL PVOID Data, IN ULONG DataSize)
	{
	...
	NTSTATUS LoadDriver(IN PCWSTR DriverServiceName)
	{
		UNICODE_STRING UnicodeString;
		RtlInitUnicodeString(&UnicodeString, DriverServiceName);
		return NtLoadDriver(&UnicodeString);
	}
	NTSTATUS UnloadDriver(IN PCWSTR DriverServiceName)
	{
		UNICODE_STRING UnicodeString;
		RtlInitUnicodeString(&UnicodeString, DriverServiceName);
	...
	NTSTATUS LoadUnloadDriver(IN PCWSTR DriverServiceName, IN OPTIONAL PCWSTR ImageFilePath, IN ULONG Type = 1, IN ULONG Start = 0xFFFFFFFF)
	{
		HANDLE Key;
		NTSTATUS status;
		if (ImageFilePath)
...
int main()
{
	...
	NTSTATUS status = NtQuerySystemInformation(SystemBasicPerformanceInformation, NULL, NULL, &ReturnLength);

	printf("SyscallId[%d]\n", GetSyscallNumber(SYSCALLID_NtQuerySystemInformation, PebBuildNumber));
	printf("Status[0x%X] Length[0x%X]\n", status, ReturnLength);
	;
	printf("Allocated[0x%p]\n", kernel32::VirtualAlloc(NULL, 0x1000, MEM_COMMIT | MEM_RESERVE, PAGE_EXECUTE_READWRITE));
